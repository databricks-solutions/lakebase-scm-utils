...
// scripts/lakebase/cut-tier.cli.ts
var cut_tier_cli_exports = {};
__export(cut_tier_cli_exports, {
  parseArgs: () => parseArgs,
  resolveFromEnv: () => resolveFromEnv
});
module.exports = __toCommonJS(cut_tier_cli_exports);
...
// scripts/lakebase/cut-tier.cli.ts
var HELP = `lakebase-cut-tier , cut a long-running tier branch (recovery for a failed create tier step)
...
${HELP}`);
  ...
  if (!instance) {
    process.stderr.write(
      `Error: no Lakebase project id (pass --instance or set LAKEBASE_PROJECT_ID in ${path2.join(projectDir, ".env")}).
