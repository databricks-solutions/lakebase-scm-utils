...
var scm_cleanup_cli_exports = {};
__export(scm_cleanup_cli_exports, {
  resolveFromEnv: () => resolveFromEnv,
  runCleanupCli: () => runCleanupCli
});
module.exports = __toCommonJS(scm_cleanup_cli_exports);
...
// scripts/lakebase/scm-cleanup.ts
var ScmCleanupError = class extends Error {
  constructor(message) {
    super(message);
...
async function runCleanup(mode, opts) {
  if (!opts.instance) throw new ScmCleanupError("cleanup requires an instance (project id).");
  const list = opts.listBranches ?? listBranches;
  const del = opts.deleteBranch ?? deleteBranch;
  const delProject = opts.deleteProject ?? deleteLakebaseProject;
  ...
  if (mode === "list") {
    for (const { b, kind } of kinds) {
      actions.push({
        ...
        reason: kind === "ephemeral" ? void 0 : `protected ${kind}`
      });
    }
  ...
  if (mode === "branches") {
    ...
    for (const { b, kind } of kinds) {
      ...
      try {
        ...
        allOk = false;
        actions.push({ resource: b.nameLeaf, kind, action: "delete", ok: false, error: err.message });
      }
    }
  ...
  if (!dryRun && opts.confirmProjectId !== opts.instance) {
    throw new ScmCleanupError(
      `project destroy requires --confirm ${opts.instance} (got ${opts.confirmProjectId ?? "nothing"}). This deletes the entire Lakebase project and every branch.`
    );
  }
  for (const { b, kind } of kinds) {
    actions.push({ resource: b.nameLeaf, kind, action: "delete", ...dryRun ? {} : { ok: void 0 } });
  }
  if (dryRun) {
  ...
  try {
    ...
    actions.push({ resource: opts.instance, kind: "project", action: "delete", ok: false, error: err.message });
...
function parseArgs(argv) {
  ...
  if (rest.length && MODES.has(rest[0])) out.mode = rest[0];
  else if (rest.length) throw new ScmCleanupError(`unknown mode "${rest[0]}" (expected: list | branches | project)`);
  return out;
}
var HELP = `lakebase-scm-cleanup: tear down a project's Lakebase SCM resources
...
async function runCleanupCli(argv) {
  let args;
  try {
    args = parseArgs(argv);
  } catch (err) {
