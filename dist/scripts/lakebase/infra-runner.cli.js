...
function buildSchemaQuery(schema) {
  ...
  const join11 = "FROM information_schema.columns c JOIN pg_tables t ON c.table_name = t.tablename AND c.table_schema = t.schemaname ";
  if (isAllSchemas(schema)) {
    return {
      text: `SELECT ${cols} ` + join11 + `WHERE ${SYSTEM_SCHEMA_FILTER} ORDER BY c.table_schema, c.table_name, c.ordinal_position`,
      values: []
    };
  }
  const one = (schema ?? "").trim() || "public";
  return {
    text: `SELECT ${cols} ` + join11 + "WHERE c.table_schema = $1 ORDER BY c.table_name, c.ordinal_position",
    values: [one]
  };
}
...
async function runInfraSuite(args) {
  ...
  if (args.junitOutput) {
    fs10.mkdirSync(path10.dirname(args.junitOutput), { recursive: true });
    fs10.writeFileSync(args.junitOutput, formatJUnit(result), "utf8");
  }
  return result;
}
