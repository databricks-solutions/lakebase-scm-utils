...
function findTemplatesDir2() {
  ...
  const here = path23.dirname(fileURLToPath2(import.meta.url));
  let dir = here;
  for (let i = 0; i < 6; i++) {
...
function findTemplatesDir3() {
  ...
  const here = path24.dirname(fileURLToPath3(import.meta.url));
  let dir = here;
  for (let i = 0; i < 6; i++) {
...
async function commitAllIfChanged(args) {
  ...
  for (const inc of args.include ?? []) {
    if (existsSync26(join29(args.cwd, inc))) {
...
function isCliEntry(importMetaUrl) {
  ...
  try {
    moduleResolved = realpathSync(fileURLToPath5(importMetaUrl));
  } catch {
    return false;
  }
