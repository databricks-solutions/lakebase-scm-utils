...
// pg_stat_activity) that identifies WHICH tool opened the connection:
//   - `consort/<consort-version>` when made under a Consort run (CONSORT_VERSION set);
//   - `scm-utils/<scm-utils-version>` when scm-utils is used directly (extension / bare CLI).
// These assert both branches + that the direct-use version resolves from the real
// package.json (so a shipped build is never `scm-utils/unknown`).
